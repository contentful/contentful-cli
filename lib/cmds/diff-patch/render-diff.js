...
export function renderModelDiff (model) {
  const diffNotEmpty = model.some(({ diff }) => diff.some(chunk => chunk.added || chunk.removed))
  if (!diffNotEmpty) {
    return log(frame(`${emojic.sparkles}  Your content types are identical ${emojic.sparkles}`))
  }
  model.forEach(renderContentTypeDiff)
}

export function renderContentTypeDiff (ct) {
  const formattedDiff = formatDiff(ct)
  if (formattedDiff.length === 1) {
    return
  }
  log(frame(formattedDiff.join(''), false, false))
}

export function formatDiff (ct) {
  ...
  return ctLines.filter(part => part !== null)
}
