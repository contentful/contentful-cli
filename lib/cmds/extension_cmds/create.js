...
async function extensionCreate (argv) {
  ...
  const mergedProperties = await mergeDescriptor(argv)
  ...
  const extensionData = {
    extension: omitBy({
      name: mergedProperties.name,
      src: mergedProperties.src,
      srcdoc: mergedProperties.srcdoc,
      fieldTypes: mergedProperties.fieldTypes.map(getFieldType)
    }, isUndefined)
  }

  const createPromise = argv.id
    ? space.createUiExtensionWithId(mergedProperties.id, extensionData)
