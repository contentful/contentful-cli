...
export async function guide () {
  ...
  // Run dev server
  log()
  log(`${highlightStyle('Almost done!')} Your Custom app has been set up on your local machine.`)
  log()
  ...
  log()
  wrappedLog(`Kill the dev server by hitting ${highlightStyle('[CTRL] + [C]')} or ${highlightStyle('[Q]')} on your keyboard.`, GUIDE_MAX_WIDTH)
  log()
  ...
  log()
  log('You can navigate to your Custom app by running:')
  log(codeStyle(`$ cd ${relative(process.cwd(), installationDirectory)}`, true))
  log()
