...
import { handleAsyncError as wrapErrorHandling } from '../../utils/async'
...
import { PATCH_FILE_HANDLER } from '../../core/events/scopes'
...
export const handler = wrapErrorHandling(handle)

const readPatchDir = helpers.readPatchDir
...
export async function handle (argv) {
  ...
  const patchFilePaths = await readPatchDir(argv.patchDir)
  if (patchFilePaths.length === 0) {
    return dispatcher.error('MISSING_PATCH_FILES', { patchDirPath: argv.patchDir })
  }
  const args = {
    spaceId,
