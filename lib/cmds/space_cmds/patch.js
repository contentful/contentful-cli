...
export async function batchPatch (space, patchDir, helpers, argv) {
  ...
  for (let patchFilePath of patchFilePaths) {
    ...
    if (!patchFile.id) return logging.logError(`No content type id provided in ${patchFilePath}`)
    if (!patchFile.patches) return logging.logError({message: `No patches provided in ${patchFilePath}`})
    ...
    await applyPatches(patchFile, contentType, helpers, logging, options)
