...
import { assertLoggedIn, assertSpaceIdProvided, assertEnvironmentIdProvided } from '../../../utils/assertions'
...
const ctVariableEscape = function (ctId) {
  ...
  return !isNaN(ctId.charAt(0)) ? `_${camelCased}` : camelCased
}
...
const generateContentTypeMigration = async function (environment, contentType) {
  ...
  try {
    ...
    // We do nothing - This means that the Content Type has no fields
  }
...
export async function generateMigration (argv) {
  ...
  await assertEnvironmentIdProvided(argv)
