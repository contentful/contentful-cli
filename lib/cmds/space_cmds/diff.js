...
export const handler = async (argv) => {
  ...
  const currentModel = await currentSpace.getContentTypes()
  const targetModel = await targetSpace.getContentTypes()

  const diffData = diffJson(currentModel.toPlainObject(), targetModel.toPlainObject())
