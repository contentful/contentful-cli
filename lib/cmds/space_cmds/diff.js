...
import {getPatchesAndDiff} from '../../core/diff/diff-patch-data'
import {renderContentTypeDiff, renderModelDiff} from '../../core/diff/render-diff'
...
export const desc = '[BETA] Diffing the content models of the current space and a target space'
...
export const diffct = async (argv) => {
  ...
  if (argv.patchDir) {
    await fsUtils.ensureDir(argv.patchDir)
  }
  ...
  const tasks = new Listr([{
    ...
    task: (ctx) => {
      return new Listr([
        ...
        {
          title: 'Writing patch files',
          task: () => {
            return new Listr(ctx.patches.map((ctPatch) => {
              const fileName = `contentful-patch-${slug(ctPatch.name)}-${Date.now()}.json`
              const fullFilePath = resolve(argv.patchDir, fileName)
              return {
  ...
  return tasks.run().then(ctx => {
    renderModelDiff(ctx.diff, renderContentTypeDiff)
  })
}
