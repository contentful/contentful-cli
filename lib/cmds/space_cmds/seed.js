...
export async function spaceSeed (argv) {
  ...
  const destination = resolve(__dirname, '..', '..', '..', 'seeds')
  await getLatestGitHubRelease('contentful/content-models', destination)

  await spaceImport({
    contentFile: resolve(destination, argv.template, 'contentful-export.json'),
    spaceId,
    managementToken: cmaToken
  })

  log()
  success(`${successEmoji} The '${argv.template}' Content model was applied to your ${highlightStyle(space.name)} (${highlightStyle(space.sys.id)}) Space.`)
}
