...
export default async function batchPatch (args, createContentfulClient, applyPatches, helpers, logging) {
  ...
  if (!options.dryRun) {
    ...
    if (!confirmed) {
      logging.log('Your patches have been applied as drafts, not published.')
    }

    const publishedCTs = changedCTs.map(ct => maybePublishContentType(ct, options.dryRun))

    await Promise.all(publishedCTs)
