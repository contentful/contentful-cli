...
export default async function finishStep (guideContext) {
  ...
  log(separator(GUIDE_MAX_WIDTH))
  log()
  wrappedLog(`Your new Space now contains ${entries.items.length} entries. Navigate to ${pathStyle(`https://app.contentful.com/spaces/${spaceId}/entries`)} to add new or change the exist ones.`, GUIDE_MAX_WIDTH)
  log()
