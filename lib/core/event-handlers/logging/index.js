...
const createLoggingStream = (events$, loggingConfig) => {
  ...
  const scopedEvents$ = events$.filter(isInScope(loggingConfig.scopes))

  const loggingData$ = scopedEvents$.map(({ payload }) => {
...
class LoggingSystem {
  ...
  connect (events$) {
    ...
    const logStream$ = Observable.merge(...loggingStreams)
