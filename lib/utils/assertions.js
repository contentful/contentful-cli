...
export async function assertEnvironmentIdProvided ({environmentId}) {
  const context = await getContext()
  if (!(environmentId || context.activeEnvironmentId)) {
    throw new PreconditionFailedError(`You need to provide a environment id. You can pass it via the ${highlightStyle('--environment-id')} parameter or by running ${highlightStyle('contentful space environment use')}`)
  }
}
