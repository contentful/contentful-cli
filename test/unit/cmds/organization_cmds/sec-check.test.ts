...
const {
  securityCheck
} = require('../../../../lib/cmds/organization_cmds/sec-check')
const {
  createPlainClient
...
function mockClient(userId, role, extra = {}) {
  const rawGet = jest.fn().mockImplementation(p => {
    ...
    if (p.includes('/organization_memberships')) {
      const items = role
        ? [{ role, user: { sys: { id: userId || 'no-user' } } }]
        : []
      return Promise.resolve({ data: { items } })
    }
    if (p.endsWith('/identity_provider')) {
      return Promise.resolve(
        extra.identity_provider || { data: { enabled: true, restricted: true } }
      )
    }
    if (p.endsWith('/security_contacts')) {
      return Promise.resolve(
        extra.security_contacts || {
          data: { items: [{ email: 'sec@example.com' }] }
        }
      )
    }
    ...
    if (p.endsWith('/audit_log/config')) {
      return Promise.resolve(
        extra.audit_log_config || { data: { enabled: true } }
      )
    }
...
test('writes default output file when -o flag is boolean true', async () => {
  ...
  const files = fs
    .readdirSync(dataDir)
    .filter(f => f.endsWith('-org123-sec-check.json'))
...
test('management-token arg overrides context token', async () => {
  ...
  await securityCheck({
    context: { managementToken: 'contextToken' },
    'organization-id': ORG_ID,
    'management-token': 'overrideToken'
  })
  expect(exitStub).not.toHaveBeenCalled()
})

test('skips sso_enforced when sso_enabled dependency fails', async () => {
  mockClient('ownerD', 'owner', {
    identity_provider: { data: { enabled: false, restricted: true } }
  })
...
test('active_tokens_with_long_expiry fails with offending long-expiry tokens present', async () => {
  mockClient('ownerE', 'owner', {
    access_tokens: {
      data: {
        items: [
          {
            revokedAt: null,
            sys: { id: 'tok1', expiresAt: '2100-01-01T00:00:00Z' }
          },
          {
            revokedAt: null,
            sys: { id: 'tok2', expiresAt: '2100-06-01T00:00:00Z' }
          }
        ]
      }
    }
