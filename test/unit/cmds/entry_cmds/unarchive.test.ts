...
const fakeClient = {
  entry: {
    ...
    unarchive: jest.fn().mockResolvedValue(unarchivedResult)
  }
}
...
beforeEach(() => {
  ...
  fakeClient.entry.unarchive.mockResolvedValue(unarchivedResult)
  mockCreatePlainClient.mockResolvedValue(fakeClient)
})

describe('entry unarchive — handler', () => {
  it('calls entry.unarchive() with the provided ID', async () => {
    await handler(baseArgv)
    expect(fakeClient.entry.unarchive).toHaveBeenCalledWith({entryId: 'entry-abc'})
  })
...
describe('entry unarchive — dry run', () => {
  it('does not call entry.unarchive() when --dry-run is set', async () => {
    ...
    expect(fakeClient.entry.unarchive).not.toHaveBeenCalled()
  })
...
describe('entry unarchive — error handling', () => {
  ...
  it('calls logError and exits when entry.unarchive throws', async () => {
    const err = Object.assign(new Error('Not Found'), {response: {status: 404}})
    fakeClient.entry.unarchive.mockRejectedValueOnce(err)
