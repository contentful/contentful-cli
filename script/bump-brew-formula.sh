...
brew bump-formula-pr --strict ${package} --url=${url} --sha256=${sha256} --no-browse --message="This PR was automatically created via a script. Contact @contentful with any questions." $DRY_RUN || { if [ "${DRY_RUN}" != "" ]; then echo "Finished ${DRY_RUN}."; else exit 1; fi; }
